...
def evaluate_model(model: str, test_turns: list) -> dict:
    ...
    for i, (date, role, content, expected_type) in enumerate(test_turns, 1):
    ...
    return {
        ...
        "avg_latency_ms": round(sum(r["latency_ms"] for r in results if r["latency_ms"] > 0) / 
                                 max(1, sum(1 for r in results if r["latency_ms"] > 0)), 1),
        "turns": results
    }
...
def main():
    ...
    for model, res in all_results.items():
        ...
        print(f"\n{model}")
        print(f"  Parse success rate: {res['parse_success_count']}/{res['total']} ({success_rate:.0f}%)")
