...
def main():
    ...
    # 6. Cross-contamination 检查（自测：scope 隔离性验证）
    print(f"\n[8] Cross-contamination 检查（scope 隔离性）：")
    try:
        import sqlite3 as _sqlite3
        import urllib.request
        import json as _json

        # 用 ollama 生成 embedding（与其他 phase3 代码一致）
        def _emb(text: str):
        ...
        disp_rows = _conn.execute(
            "SELECT id, condition_text, scope FROM l1_dispositions WHERE is_active = 1 AND condition_embedding IS NOT NULL"
        ).fetchall()
        _conn.close()

        if not disp_rows:
            print("    ⚠️  无 active disposition，无 scope 可检")
        else:
            import struct, math
            def _deserialize(blob):
                ...
                return list(blob) if isinstance(blob, (tuple, list)) else blob
            def _cosine(a, b):
            ...
            tested = 0
            for row in disp_rows[:30]:  # 最多抽30条
                try:
                    q_emb = _emb(row["condition_text"])
                except Exception:
                    continue
                # 每次重新打开连接（避免游标 stale）
                ...
                scope = row["scope"]
                cross_rows = _conn2.execute(
                    "SELECT id, condition_text, scope, condition_embedding FROM l1_dispositions "
                    "WHERE is_active = 1 AND condition_embedding IS NOT NULL AND scope != ?",
                    (scope,)
                ).fetchall()
                _conn2.close()
                best_cross = None
                ...
                for cr in cross_rows:
                    try:
                        emb = _deserialize(cr["condition_embedding"])
                        ...
                        continue
                tested += 1
                if best_cross and best_sim > 0.75:
                    contamination_found += 1
                    print(f"    ⚠️  [{scope} → {best_cross['scope']}] sim={best_sim:.2f} | {row['condition_text'][:35]}")
            if tested == 0:
                print("    ⚠️  embedding 服务不可用，跳过")
            elif contamination_found == 0:
                print(f"    ✅ 无 cross-contamination（测了 {tested} 条，scope 隔离正常）")
            else:
                print(f"    ❌ 发现 {contamination_found} 次 contamination（{tested} 条中）")
    except Exception as e:
        import traceback
        print(f"    ⚠️  检查跳过: {e}")
        traceback.print_exc()
