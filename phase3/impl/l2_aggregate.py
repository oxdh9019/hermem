...
from .config import DB_PATH, SIM_THRESHOLD_JOIN, SIM_THRESHOLD_MERGE, SCENE_DORMANT_DAYS
from .utils import (
    cosine_sim, serialize_vec, deserialize_vec,
    get_embeddings_batch, db_query, db_query_dict,
)
    ...
    for i, s1 in enumerate(scenes):
        ...
        for s2 in scenes[i + 1:]:
            ...
            if sim > SIM_THRESHOLD_MERGE:
                ...
                new_summary = _regenerate_scene_summary(
                ...
                conn.execute("""
                    ...
                    WHERE id=?
                """, (json.dumps(merged_refs), new_summary, len(merged_refs),
                      datetime.now().isoformat(), s1["id"]))
