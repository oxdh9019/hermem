...
class TestActivationScore_ErrorFactor:
    ...
    def test_error_count_zero_suppresses(self):
        ...
        score, f_time, ef = calculate_activation_score(sim=0.8, error_count=0, last_error_at=None)
    ...
    def test_error_count_one_neutral(self):
        ...
        score, f_time, ef = calculate_activation_score(sim=0.8, error_count=1, last_error_at=None)
        assert ef == 1
        assert score == pytest.approx(0.8)

    def test_error_count_capped(self):
        """error_count 超过 cap 时被截断"""
        score5, _, ef5 = calculate_activation_score(sim=0.8, error_count=5, last_error_at=None)
    ...
    def test_error_count_below_cap(self):
        ...
        score3, _, ef3 = calculate_activation_score(sim=0.7, error_count=3, last_error_at=None)
        score4, _, ef4 = calculate_activation_score(sim=0.7, error_count=4, last_error_at=None)
        assert ef3 == 3
        assert ef4 == 4
...
class TestActivationScore_TimeDecay:
    ...
    def test_no_last_error_fresh(self):
        ...
        score, f_time, _ = calculate_activation_score(sim=0.9, error_count=3, last_error_at=None)
        assert f_time == 1.0
...
class TestActivationScore_Combined:
    ...
    def test_sim_dominates_when_time_fresh(self):
        ...
        score = calculate_activation_score(sim=0.9, error_count=5, last_error_at=recent)[0]
        # 0.9 * ~0.9 * 5 ≈ 4.05
        assert score > 3.5
    ...
    def test_old_low_sim_minimal(self):
        ...
        score = calculate_activation_score(sim=0.3, error_count=0, last_error_at=old)[0]
...
class TestActivationScore_Boundaries:
    ...
    def test_sim_zero_returns_zero(self):
        """sim=0 → score=0"""
        score, _, _ = calculate_activation_score(sim=0.0, error_count=5, last_error_at=None)
        assert score == 0.0

    def test_sim_negative_unchanged(self):
        """负相似度数学上可能（取决于 cosine_sim），函数不拦截"""
        score, _, _ = calculate_activation_score(sim=-0.1, error_count=2, last_error_at=None)
