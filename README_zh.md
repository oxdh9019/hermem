...
**V6 全部完成**（2026-06-12，7 sprint + 3 个 P0 修复）。4 信号触发 + RRF k=60 融合 + 时间通道 + Sprint 1.5 桥层修复 + Sprint 2 预测性召回（qwen3.5:4b）+ Sprint 3 可解释包装 + Sprint 4 评测框架（20 条 ground-truth、4 场景、CI 回归）。SPEC §0 5 目标全部达成；baseline Recall@5 38.2% → 66.2% (+28%)。总览见 `phase3/v6/eval/v6-overview.md`。
...
| **V6** | **按需触发 + RRF 融合 + 时间通道 + 评测** | 4 信号 `_v6_should_trigger()`（medium_accumulated > anchor > temporal > intent > frequency）、RRF（k=60）vec+BM25 双路融合、9 条 regex 时间解析器、`hermes hermem stats` CLI、`recall_outcome` 行为回路、Sprint 1.5 桥层 float→int 修复 `medium_tracker`、Sprint 2 预测性召回（qwen3.5:4b）、Sprint 3 可解释包装 + reflect API、Sprint 4 评测框架（20 条 ground-truth + 概念权重重排 + 周报 + CI）。规划见 `phase3/v6/SPEC.md` v2.0（7 sprint 全部 ✅，2026-06-12） |
...
│   └── eval/                    # 评估脚本
│
# 注：Hermes gateway 插件封装（`plugins/memory/hermem/`）位于独立的
# hermes-agent checkout 中 — 见 §Hermes Agent 集成（桥接器）章节。
```
...
- `phase3/scripts/test_v5_e2e.py`：端到端测试（8/8 通过，2026-06-12 复核）
- `plugins/memory/hermem/cli.py`：`hermes memory health` + `hermes memory rebuild`

**Phase B 状态：** V5 Phase B（中置信累积触发）已被 V6 Sprint 1 的 `medium_accumulated` 信号取代 — 见 §V6 4 信号触发 信号 #1。
...
| Sprint 1.5（桥层 float→int 修复） | 1 | ✅ | `eval/sprint1-summary.md` §4 偏差 5 |
| Sprint 2（预测性召回） | 7 | ✅ | `eval/sprint2-summary.md` |
| Sprint 3（可解释包装 + reflect API） | 6 | ✅ | `eval/sprint3-summary.md` |
| Sprint 4（评测框架 + 排序权重 + 周报 + CI） | 7 | ✅ | `eval/sprint4-summary.md` |

**测试数（2026-06-12 verify-on-disk）：** 273/273 全部通过，覆盖 `phase3/tests/` + `phase3/v5.5/tests/` + `phase3/v6/tests/`。分目录：`phase3/tests/` 138、`phase3/v5.5/tests/` 18、`phase3/v6/tests/` 117（Sprint 1-4 累计）。`hermes hermem health`：drift 7（2357 meta vs 2350 npy = 7 陈旧），非 P0，`hermes memory rebuild` 可修。

完整规划：`phase3/v6/SPEC.md` v2.0。各 sprint summary：`phase3/v6/eval/sprint{0,05,1,2,3,4}-summary.md`。V6 收尾总览：`phase3/v6/eval/v6-overview.md`（5KB — 7 sprint + 5 目标 + baseline 38.2%→66.2%）。
...
**Sprint 4 增补（2026-06-12）：** 在 disposition 提升之上叠加概念权重重排 — `hermem_search` 主路径生效，基于 L2 场景聚类计算 query 在概念空间的权重分布，与 V4.5 disposition 提升串联。详见 `phase3/v6/eval/sprint4-summary.md` §1 任务 4.5 + `phase3/v6/SPEC.md` §3 修订段。
...
| 单元测试 | ✅ 根 pytest 可发现 273 项（impl 138 + v5.5 18 + v6 117） |
| CI/CD | ❌ 无 |
