...
**V5 已上线**（2026-05-27）。1637 个 chunk 已生成 bge-m3 向量，分层阈值（高≥0.70 / 中≥0.65），会话级去重，注入格式 `[自动回忆 - 相似度 X.XX]`。
...
**当前数据：1706 个向量（1637 个 chunk），22 个 disposition，80 个 L2 场景**（截至 2026-05-27）。
...
- `phase3/scripts/batch_compute_embeddings.py`：预计算全部 1637 个 chunk 向量
- `phase3/scripts/test_v5_e2e.py`：端到端测试（7/8 通过）

**Phase B 待完成：** 中置信累积触发、增量 embedding 验证
...
| **V5 主动检索** | ✅ Phase A — 向量搜索、注入、去重完成。Phase B 待完成（中置信累积、增量 embedding） |
| 意图分类器 | ✅ 13 种意图 + 双层架构 |
