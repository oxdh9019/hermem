...
| **V5.5** | **元认知 + 冲突 + 遗忘** | L4 反思 cron，记忆冲突协商，生物学启发的主动遗忘 |
...
## V5.5 — 元认知、冲突与遗忘（2026-05-28）

V5.5 新增三个高阶记忆功能：

### L4 反思层（元认知）

每日 Cron（02:00）读取前一天 `prediction_errors` 记录，用 LLM 归纳用户交互模式的元记忆，写入 `l4_reflections` 表，TTL 14 天。
...
L1 事实持久化时，检测与高置信 disposition 的冲突（相似度 > 0.75 + 语义矛盾）。写入 `pending_conflicts` 表，生成用户询问消息。
...
- **睡眠巩固**（每周）：高频召回（usage_count > 5，last_used_at ≥ 7 天）→ LLM 归纳 → user_profile.md
- **主动降级**（每周）：30 天未召回且 confidence < 0.6 → 归档

**使用追踪：** `impl/usage_tracker.py` 在每次 retrieve() 调用时异步更新 `usage_count`/`last_used_at`。
...
```bash
# 每周日 02:30 — L4 反思 + 睡眠巩固 + 主动降级
hermes cron create "30 2 * * 0" \
  --name "Hermem V5.5 每周记忆综合" \
...
| **V5.5 元认知** | ✅ L4 反思 cron + LLM fallback + 14 天 TTL |
| **V5.5 冲突协商** | ✅ detect + pending + resolve 完整链路 |
| **V5.5 主动遗忘** | ✅ usage_tracker 连接 + sleep consolidation + active demotion |
| 意图分类器 | ✅ 13 种意图 + 双层架构 |
